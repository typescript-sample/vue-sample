...
<script lang="ts">
import {AuthInfo, AuthResult, getMessage, handleCookie, initFromCookie, Status} from 'authentication-component';
...
export default class SigninComponent extends Vue {
  ...
  signin(event) {
    ...
    authenticator.authenticate(this.user).then((result: AuthResult) => {
      const s = result.status;
      // tslint:disable-next-line:triple-equals
      if (s === status.success || s === status.success_and_reactivated) {
        handleCookie('data', this.user, remember, this.cookieService, 60 * 24 * 3, Base64);
        /*
